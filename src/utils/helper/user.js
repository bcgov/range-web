...
export const getUserRoleObj = (user) => {
  const { roleId } = user || null;
  if (roleId) {
    return {
      id: roleId,
      description: USER_ROLE[roleId],
    };
  }
  return {
    id: -1,
...
const getUserFamilyName = (user) => user && user.familyName && capitalize(user.familyName);

const getUserGivenName = (user) => user && user.givenName && capitalize(user.givenName);
...
export const canManageEmails = (user) => user && user.permissions && user.permissions.find((p) => p.id === 9);

export const canAssignRoles = (user) => user && user.permissions && user.permissions.find((p) => p.id === 11);

export const canReadAll = (user) => user && user.permissions && user.permissions.find((p) => p.id === 1);
