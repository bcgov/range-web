...
export const isUserAdmin = user => (
  isUserActive(user) && user.roles
    && (user.roles.indexOf(USER_ROLE.ADMINISTRATOR) >= 0)
);

export const isUserRangeOfficer = user => (
  isUserActive(user) && user.roles
    && (user.roles.indexOf(USER_ROLE.RANGE_OFFICER) >= 0)
);

export const isUserAgreementHolder = user => (
  isUserActive(user) && user.roles
    && (user.roles.indexOf(USER_ROLE.AGREEMENT_HOLDER) >= 0)
);
