...
export const getUserfullName = (user = {}) => (
  user.givenName && user.familyName && `${user.givenName} ${user.familyName}`
);

export const isUserActive = (user = {}) => user.active;

export const isUserAdmin = (user = {}) => (
  isUserActive(user) && user.roles && (user.roles.indexOf(USER_ROLE.ADMINISTRATOR) >= 0)
);

export const isUserRangeOfficer = (user = {}) => (
  isUserActive(user) && user.roles && (user.roles.indexOf(USER_ROLE.RANGE_OFFICER) >= 0)
);

export const isUserAgreementHolder = (user = {}) => (
  isUserActive(user) && user.roles && (user.roles.indexOf(USER_ROLE.AGREEMENT_HOLDER) >= 0)
);
