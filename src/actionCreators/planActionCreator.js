...
export const createRUPInvasivePlantChecklist =
  (planId, invasivePlantChecklist) => (dispatch, getState) => {
    return axios
      .post(
...
export const createRUPStatusRecord =
  (plan, newStatus, note) => (dispatch, getState) => {
  ...
  const makeRequest = async () => {
    try {
      ...
      const additionalRequirementsMap =
        getAdditionalRequirementsMap(getState());
      const managementConsiderationsMap =
        getManagementConsiderationsMap(getState());
      ...
      const newPastureIdsMap = normalizePasturesWithOldId(
        pastures,
        newPastures,
      );
      ...
      const invasivePlantChecklist =
        copyInvasivePlantChecklistToCreateAmendment(plan);
