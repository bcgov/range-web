...
const mockAHUser = {
  id: 1,
  roles: [USER_ROLE.AGREEMENT_HOLDER]
}
...
beforeEach(() => {
  ...
  store.dispatch(storeUser(mockAHUser))
  mockAxios.reset()
})
...
describe('Integration testing', () => {
  it('Component initializes properly', async () => {
    ...
    mockPlan.status.code = 'D' // set the status as Draft
    ...
    const wrapper = mount(
      <Provider store={store}>
        <MemoryRouter initialEntries={[`/range-use-plan/${mockPlan.id}`]}>
          <RangeUsePlanWithRouter user={mockAHUser} />
        </MemoryRouter>
      </Provider>
    )
    ...
    const normalizedPlan = normalize(mockPlan, schema.plan).entities.plans[
      mockPlan.id
    ]
