...
import { writeTitle, drawHorizontalLine, writeText, writeFieldText } from './common';
...
export const writeGrazingSchedules = (doc, plan) => {
  ...
  if (grazingSchedules.length === 0) {
    currY += 2;
    currY = writeText({
      doc, text: 'No schedule provided.',
      x: startX, y: currY + 0.5,
      fontColor: grayColor,
    });
  }
  ...
  grazingSchedules.map((gs) => {
    currY += 4;
    const { year, narative, grazingScheduleEntries, authorizedAUMs, crownTotalAUMs } = gs;
    currY = writeText({
      doc, text: `${year} Grazing Schedule`,
      x: startX, y: currY,
    ...
    currY = writeText({
      doc, text: `Authorized AUMs:  ${handleNullValue(authorizedAUMs, 'N/P')}   Total AUMs:  ${handleNullValue(crownTotalAUMs, 'N/P')}`,
      x: contentEndX - 0.5, y: currY,
      fontStyle: 'bold',
      hAlign: 'right',
    });
