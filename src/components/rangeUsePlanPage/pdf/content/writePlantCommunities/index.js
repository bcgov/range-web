...
export const writePlantCommunities = (doc, y, pasture) => {
  ...
  currY = writeText({
    doc, text: `Plant Communities: (${pastureName})`,
    x: startX, y: currY,
  ...
  plantCommunities.map((pc) => {
    ...
    currY = writeText({
      doc, text: `Plant Community: ${handleNullValue(communityTypeName)}`,
      x: startX + marginRight, y: currY,
      fontSize: fieldTitleFontSize + 0.5, fontStyle: 'bold', fontColor: primaryColor,
    });
    currY += 5;
    ...
    currY = writeFieldText(doc, 'Pasture Notes', notes, startX + marginRight, currY, contentWidth - marginRight);
    currY += 7;
    currY = writeFieldText(doc, 'Plant Community URL', url, startX + marginRight, currY, contentWidth - marginRight);
    currY += 7;
    currY = writeFieldText(doc, 'Purpose of Actions', purposeOfAction, startX + marginRight, currY, contentWidth - marginRight);
