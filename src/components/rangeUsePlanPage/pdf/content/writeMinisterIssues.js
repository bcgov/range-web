...
export const writeMinisterIssuesAndActions = (doc, plan) => {
  ...
  if (ministerIssues.length === 0) {
    currY = writeText({
      doc,
      text: 'No minister issue provided.',
      x: startX,
      y: currY,
      fontColor: grayColor
    })
      ...
      ministerIssueActions &&
        ministerIssueActions.map(mia => {
          ...
          if (ministerIssueActionType.name === 'Timing') {
            const noGrazeStartDate = createDateWithMoment(
              ngStartDay,
              ngStartMonth
            )
            const noGrazeEndDate = createDateWithMoment(ngEndDay, ngEndMonth)
            const formatedNoGrazeStartDate = formatDateFromServer(
              noGrazeStartDate,
              false
            )
            const formatedNoGrazeEndDate = formatDateFromServer(
              noGrazeEndDate,
              false
            )
