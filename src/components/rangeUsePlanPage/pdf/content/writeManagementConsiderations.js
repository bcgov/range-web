...
export const writeManagementConsiderations = (doc, plan) => {
  ...
  currY = writeText({
    doc, text: 'Content in this section is non-legal and is intended to provide additional information about management within the agreement area.',
    x: startX, y: currY,
    fontColor: grayColor,
  });
  currY += marginBottom;
  ...
  currY = Math.max(
    writeText({
      doc, text: 'Considerations',
      x: startX, y: momentCurrY,
      fontSize: fieldTitleFontSize, fontStyle: 'bold',
    }),
    writeText({
      doc, text: 'Details',
      x: startX + detailStartX, y: momentCurrY,
      fontSize: fieldTitleFontSize, fontStyle: 'bold',
    }),
  );
  currY += marginBottom;
  ...
  managementConsiderations.map((mc) => {
    ...
    currY = Math.max(
      writeText({
        doc, text: considerationTypeName,
        x: startX, y: momentCurrY,
        fontSize: fieldTitleFontSize, fontColor: grayColor,
    ...
    writeText({
      doc, text: 'URL: ',
      x: startX + detailStartX, y: currY,
      fontSize: fieldTitleFontSize, fontStyle: 'bold',
      cusContentWidth: contentWidth - detailStartX,
    });
    currY = writeText({
      doc, text: url,
      x: startX + detailStartX + 11, y: currY,
      fontSize: fieldTitleFontSize, fontColor: grayColor,
