...
export const writePastures = (doc, plan) => {
  ...
  pastures.map(pasture => {
    ...
    currY = writeText({
      doc,
      text: `Pasture: ${handleNullValue(name)}`,
      x: startX,
      y: currY,
    ...
    currY = Math.max(
      writeFieldText(
        doc,
        'Allowable AUMs',
        `${handleNullValue(allowableAum)}`,
        startX,
        momentCurrY
      ),
      writeFieldText(
        doc,
        'Private Land Deduction',
        `${handleNullValue(pldPercent)}`,
        oneThirdWidth,
        momentCurrY
      ),
      writeFieldText(
    ...
    currY = writeFieldText(
      doc,
      'Pasture Notes (non legal content)',
      notes,
      startX,
      currY
    )
