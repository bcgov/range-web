...
export const writePlantCommunityActions = (doc, y, pc) => {
  ...
  currY += 7;
  currY = writeText({
    doc, text: `Plant Community Actions (${handleNullValue(communityTypeName)})`,
  ...
  currY = Math.max(
    writeText({
      doc, text: 'Action',
      x: startX + marginRight, y: momentCurrY,
      fontStyle: 'bold',
    }),
    writeText({
      doc, text: 'Details',
  ...
  plantCommunityActions.map((pca) => {
    ...
    const period = `${
      getMonthAndDateIntegers(noGrazeStartMonth, noGrazeStartDay)
    } - ${
      getMonthAndDateIntegers(noGrazeEndMonth, noGrazeEndDay)
    }`;
    currY = writeText({
      doc, text: period,
      x: startX + detailStartX + 30, y: currY,
      fontColor: grayColor,
