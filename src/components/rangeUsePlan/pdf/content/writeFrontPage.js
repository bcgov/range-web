...
export const writeFrontPage = (doc, plan, logoImage) => {
  ...
  doc.addImage(logoImage, 'PNG', halfPageWidth - (imageW / 2), currY, imageW, 214 / divide);
  currY += 50;

  writeText({
    doc, text: 'Range Use Plan',
    x: halfPageWidth, y: currY,
    fontSize: sectionTitleFontSize + 9, fontStyle: 'bold', fontColor: primaryColor,
    hAlign: 'center',
  });
  currY += 20;

  writeText({
    doc, text: `#${forestFileId}`,
    x: halfPageWidth, y: currY,
    fontSize: sectionTitleFontSize + 1, fontStyle: 'bold',
    hAlign: 'center',
  });
  currY += 8;

  writeText({
    doc, text: `${formatDateFromServer(asd)} - ${formatDateFromServer(aed)}`,
    x: halfPageWidth, y: currY,
    fontSize: sectionTitleFontSize + 1, fontColor: grayColor,
    hAlign: 'center',
  });
  currY += margin;

  writeText({
    doc, text: 'Confirmed by Agreement Holder(s):',
    x: halfPageWidth, y: currY,
    fontSize: sectionTitleFontSize + 1, fontStyle: 'bold',
    hAlign: 'center',
  });
  ...
  clients.map((client) => {
    ...
    writeText({
      doc, text: `${getClientFullName(client)} - ${confirmationDate}`,
      x: halfPageWidth, y: currY,
      fontSize: sectionTitleFontSize + 1, fontColor: grayColor,
      hAlign: 'center',
    });
  ...
  writeText({
    doc, text: 'Confirmed by District Manager:',
    x: halfPageWidth, y: currY,
    fontSize: sectionTitleFontSize + 1, fontStyle: 'bold',
    hAlign: 'center',
  });
  ...
  doc.line(halfPageWidth - halfOfLineW, currY, halfPageWidth + halfOfLineW, currY);
  writeText({
    doc, text: 'Signature',
    x: halfPageWidth, y: currY + 3,
    fontSize: fieldTitleFontSize,
    hAlign: 'center',
  });

  doc.line(halfPageWidth + halfOfLineW + interval, currY, halfPageWidth + fromCenterToStartLength, currY);
  writeText({
    doc, text: 'Date',
    x: halfPageWidth + fromCenterToStartLength - halfOfLineW, y: currY + 3,
    fontSize: fieldTitleFontSize,
