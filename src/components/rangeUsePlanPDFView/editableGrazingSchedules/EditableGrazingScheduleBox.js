...
class EditableGrazingScheduleBox extends Component {
  static propTypes = {
    schedule: PropTypes.shape({ grazingScheduleEntries: PropTypes.array })
      .isRequired,
  ...
  handleScheduleEntryDelete = entryIndex => {
    ...
    const grazingSchedule = { ...schedule }
    const [deletedEntry] = grazingSchedule.grazingScheduleEntries.splice(
      entryIndex,
      1
    )
    ...
    if (planId && scheduleId && entryId && !uuid.isUUID(entryId)) {
      ...
      // or delete the entry saved only in Redux
      onDeleted()
    }
  }
  ...
  render() {
    ...
    const copyOptions =
      yearOptions.map(o => ({
        ...o,
        onClick: this.onScheduleCopyClicked(o)
      })) || []
        ...
        headerRight={
          <Dropdown
            ...
            <Dropdown.Menu>
              ...
              />
              <Dropdown.Item onClick={this.openDeleteScheduleConfirmationModal}>
                Delete
              </Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
