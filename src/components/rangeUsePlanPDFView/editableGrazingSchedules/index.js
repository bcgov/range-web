...
export class EditableGrazingSchedules extends Component {
  ...
  getInitialYearOptions = () => {
    ...
    const { planStartDate, planEndDate, grazingSchedules: grazingScheduleIds } =
      plan || {}
    if (planStartDate && planEndDate) {
      ...
      return utils
        ...
        .filter(option => {
          ...
          const grazingSchedules = grazingScheduleIds.map(
            id => grazingSchedulesMap[id]
          )
  ...
  onScheduleClicked = scheduleIndex => () => {
    this.setState(prevState => {
      const newIndex =
        prevState.activeScheduleIndex === scheduleIndex ? -1 : scheduleIndex
      return {
  ...
  handleScheduleCopy = (year, sId) => {
    ...
    const copiedGrazingScheduleEntries = schedule.grazingScheduleEntries.map(
      e => {
        const { id, grazingScheduleId, ...entry } = e
        // replace the first 4 characters with the new year
        const dateIn =
          entry.dateIn &&
          typeof entry.dateIn === 'string' &&
          `${year}${entry.dateIn.slice(4)}`
        const dateOut =
          entry.dateOut &&
          typeof entry.dateOut === 'string' &&
          `${year}${entry.dateOut.slice(4)}`
  ...
  handleScheduleDelete = (schedule, scheduleIndex) => {
    ...
    if (planId && scheduleId && !uuid.isUUID(scheduleId)) {
      ...
      // or delete the schedule saved in Redux store
      onDeleted()
    }
  }
  ...
  addGrazingScheduleInStore = grazingSchedule => {
    ...
    this.setState({
      yearOptions: currYearOptions.filter(
        o => o.value !== grazingSchedule.year
      ),
      activeScheduleIndex: newGrazingSchedules.findIndex(
        s => s.year === grazingSchedule.year
      )
    })
  }
  ...
  renderSchedule = (schedule, scheduleIndex) => {
    const { plan, usage, references, pasturesMap } = this.props
  ...
  render() {
    ...
    const grazingSchedules =
      grazingScheduleIds &&
      grazingScheduleIds.map(id => grazingSchedulesMap[id])
