...
export const writeMonitoringAreas = (doc, y, pc) => {
  ...
  const { name, communityType, monitoringAreas } = pc
  let currY = y
  ...
  currY = writeText({
    doc,
    text: `Monitoring Areas (${handleNullValue(communityTypeName)})`,
  ...
  monitoringAreas.map(ma => {
    ...
    currY = Math.max(
      writeFieldText(
        ...
        momentCurrY,
        contentWidth - moreMarginRight
      ),
      writeFieldText(
        doc,
        'Latitude/Longitude',
        `${handleNullValue(latitude)}, ${handleNullValue(longitude)}`,
        halfPageWidth,
        momentCurrY,
        contentWidth - moreMarginRight
      )
