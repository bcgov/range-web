...
export const writeInvasivePlantChecklist = (doc, plan) => {
  const { startX, afterHeaderY, contentWidth, grayColor } = doc.config
  ...
  if (beginInUninfestedArea) {
    currY = writeText({
      doc,
      text:
        '    •    Any work will being in un-infested areas before moving to infested locations',
      x: startX,
      y: currY,
      fontColor: grayColor
    })
  ...
  if (undercarrigesInspected) {
    ...
    currY = writeText({
      doc,
      text:
        'Clothing and vehicle/equipment undercarriages will be regularly inspected for plant parts or propagules if working in an area known to contain invasive plants',
      x: startX + marginLeft,
      y: currY,
      fontColor: grayColor,
