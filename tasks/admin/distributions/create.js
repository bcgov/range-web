  ...
  for (index in release.distributions) {
    ...
    (function (distribution) {
      ...
      gulp.task(task.meteor, function () {
        ...
        gulp
          .src(release.templates.meteor[distLowerCase])
          .pipe(plumber())
          .pipe(flatten())
      ...
      if (distribution == 'CSS') {
        ...
        gulp.task(task.repo, function () {
          ...
          definitions = gulp
            ...
            .pipe(gulp.dest(outputDirectory));
          themeImport = gulp
            .src('src/semantic.less', { base: 'src/' })
            .pipe(gulp.dest(outputDirectory));
          themeImport = gulp
            .src('src/theme.less', { base: 'src/' })
            .pipe(gulp.dest(outputDirectory));
          themeConfig = gulp
            .src('src/theme.config.example', { base: 'src/' })
            .pipe(gulp.dest(outputDirectory));
          siteTheme = gulp
            .src('src/_site/**/*', { base: 'src/' })
            .pipe(gulp.dest(outputDirectory));
          themes = gulp
            .src('src/themes/**/*', { base: 'src/' })
            .pipe(gulp.dest(outputDirectory));
          return mergeStream(
            definitions,
            themeImport,
