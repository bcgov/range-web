  ...
  addDerivedValues: function (config) {
    ...
    for (folder in config.paths.source) {
      if (config.paths.source.hasOwnProperty(folder)) {
        sourcePaths[folder] = path.resolve(
          path.join(configPath, config.base, config.paths.source[folder]),
        );
      }
    }
    for (folder in config.paths.output) {
      if (config.paths.output.hasOwnProperty(folder)) {
        outputPaths[folder] = path.resolve(
          path.join(configPath, config.base, config.paths.output[folder]),
        );
      }
    }
    ...
    config.paths.clean = path.resolve(
    ...
    config.paths.assets = {
      ...
      uncompressed:
        ...
        path
          .relative(
            config.paths.output.uncompressed,
            config.paths.output.themes,
          )
          .replace(/\\/g, '/'),
      compressed:
        './' +
